...
struct BufferMemories {
    std::vector<VkBuffer> bufferID;
    std::vector<VkDeviceMemory> memoryID;
    //maybe should be 
    /*
    std::vector<BufferMemory>
    */
};
...
struct Settings {
    ...
    bool GraphicShader = true;
    bool DebugLog = false;
};

struct ModelTextureSets {
...
public:
    ...
    void SetModelMatrixPush(const Matrix4& modelMatrix,const Matrix4& modelMatrix2);
    void SetCameraUBO(const Matrix4& projection, const Matrix4& view);
    void SetGlightingUBO(const GlobalLightning& glightning);
...
private:
    ...
    std::vector<LoadModelOutput> LoadModelOutputs;
    //std::vector<Vertex> vertices;
    //std::vector<uint32_t> indices;
    ...
    std::vector<BufferMemory> vertexBuffer;

    std::vector<BufferMemory> indexBuffer;

    BufferMemories cameraBuffer;

    BufferMemories glightningBuffer;
    ...
    void updateGlightningUniformBuffer(uint32_t currentImage);
    VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags);
    void createRenderPass();
    void createDescriptorSetLayout();
    void createGraphicsPipeline(const char* vFilename_, const char* fFilename_,const char* gFilename_,VkPipeline &pipelineID);
    ...
    void loadModel(const char* filename, LoadModelOutput& ModelOutputs);
    void createVertexBuffer(LoadModelOutput& ModelOutput, BufferMemory& vertexBuffer);
        /// A helper function for createVertexBuffer()
        uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties);
    void createIndexBuffer(LoadModelOutput& ModelOutput, BufferMemory& indexBuffer);
    void createIndexandVertexBuffer(LoadModelOutput& ModelOutput, BufferMemory& indexBuffer, BufferMemory& vertexBuffer);
    void createUniformBuffers(VkDeviceSize bufferSize, BufferMemories& buffers);
    void createDescriptorPool(VkDescriptorPool& descriptorPool_);
    void createDescriptorSets(Sampler2D_data& textureData, Descriptor& descriptor);
    ...
    //std::vector<Sampler2D_data> textures;
    ...
    CameraUBO cameraUBO;
    GlobalLightning glightningUBO;
    ModelMatrixPushConst modelMatrixPushConst[2];
